 ...
 */
public class WebLogicClusterForAixThread extends WebLogicCluster{

	public WebLogicClusterForAixThread(ServerModel server,
	...
	public void run() {
		...
		try {
			...
			if (AixCheck.canUseRmsock()) {
				// 使用rmsock方式精确判断端口，程序是否已启动  (精确查询)
				processId=Tool.findPidByPortAix(server_port);
			}else{
				//使用grep查找启动命令行 （模糊查询） 例： 启动命令 ../startManagedWebLogic.sh {SERVER_NAME} {ADMIN_URL}
				pids= Tool.findPidByCommandAix("startManagedWebLogic.sh "+servicesName);
			}
			if(processId!=null || (pids!=null && pids.size()>0)){
				csmStatus.setCode(-1);
			...
			boolean checkAdminStatus = false;//检查管理服务状态,判断是否已启动
